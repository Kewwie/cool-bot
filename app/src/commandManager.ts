...
export class CommandManager {
	...
	loadPrefix(command: PrefixCommand) {
		...
		for (let alias of command.config.alies) {
	...
	async register(commands: SlashCommand[], guildId: string) {
		...
		console.log(
			`Successfully reloaded ${data.length} (/) commands in ${guildId}`
		);
	}
	...
	async onInteraction(interaction: any) {
		if (interaction.isChatInputCommand()) {
			let command = this.client.SlashCommands.get(
			...
			if (interaction.guildId) {
				let guildConfig =
					await this.client.DatabaseManager.getGuildConfig(
						interaction.guildId
					);
				if (command.premission_level) {
					...
					if (guildConfig?.permissionLevels) {
						if (
							guildConfig.permissionLevels[interaction.userId] >=
							command.premission_level
						) {
						...
						interaction.member.roles.cache.forEach(async (role) => {
							if (
								guildConfig.permissionLevels[role.id] >=
								command.premission_level
							) {
								hasHigherPermission = true;
							}
					...
					if (!hasHigherPermission) {
						await interaction.reply({
							content: `You need permission level ${command.premission_level} to use this command!`,
							ephemeral: true,
						});
						return;
					}
	...
	async onMessage(message: Message) {
		...
		if (message.guildId) {
			let guildConfig = await this.client.DatabaseManager.getGuildConfig(
				message.guildId
			);
			if (command.premission_level) {
				let hasHigherPermission = false;
				...
				if (guildConfig?.permissionLevels) {
					if (
						guildConfig.permissionLevels[message.author.id] >=
						command.premission_level
					) {
						hasHigherPermission = true;
					}
